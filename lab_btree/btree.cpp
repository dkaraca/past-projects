...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
	...
	if(child->children.size()>0){
		auto it1 = child->children.end();
		auto it2 = mid_child_itr;
		new_right->children.assign(it2,it1);
		new_left->children.erase(it2,it1);
	}
	auto it3 = child->elements.end();
	auto it4 = mid_elem_itr;
	new_right->elements.assign(it4+1,it3);
	new_left->elements.erase(it4,it3);
}
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
	...
	if(subroot->is_leaf){
		...
		BTreeNode* pointer = subroot->children[first_larger_idx];
		insert(pointer,pair);
		if(pointer->elements.size() >= order) 
