...
animation filler::fill( PNG & img, int x, int y,
	...
	int tgtW = img.width();	
	int tgtH = img.height();
	vector<vector<bool>> visited(tgtW,vector<bool>(tgtH));
	for(int i = 0; i<tgtW; i++){
		for(int j = 0; j<tgtH; j++){
			visited.at(i).at(j) = false;
		}
	}
	RGBAPixel startPixel = *img(x,y);
	RGBAPixel* currentPixel = img(x,y);
	//startPixel = img(x,y);
	//startPixel.red =  img(x,y)->red;
	//startPixel.blue =  img(x,y)->blue;
	//startPixel.green = img(x,y)->green;
	//startPixel.alpha = img(x,y)->alpha;
	int currentXCoord, currentYCoord, distance, done;
	done = 0;
	while(!xCoord.isEmpty() && !yCoord.isEmpty()){
		currentXCoord = xCoord.remove();
		currentYCoord = yCoord.remove();
		if(currentXCoord<tgtW && currentXCoord>=0 && currentYCoord<tgtH && currentYCoord>=0){
		//currentPixel->red = img(currentXCoord,currentYCoord)->red;
		//currentPixel->blue = img(currentXCoord,currentYCoord)->blue;
		//currentPixel->green = img(currentXCoord,currentYCoord)->green;
		//currentPixel->alpha = img(currentXCoord,currentYCoord)->alpha;
		currentPixel = img(currentXCoord,currentYCoord);
		distance = pow(currentPixel->red - startPixel.red,2) + pow(currentPixel->blue - startPixel.blue,2) + pow(currentPixel->green - startPixel.green,2);
		
			if((visited.at(currentXCoord).at(currentYCoord) == false) && (distance<=tolerance)){
				visited.at(currentXCoord).at(currentYCoord) = true;
				currentPixel->red = fillColor(currentXCoord,currentYCoord).red;
				currentPixel->blue = fillColor(currentXCoord,currentYCoord).blue;
				currentPixel->green = fillColor(currentXCoord,currentYCoord).green;
				currentPixel->alpha = fillColor(currentXCoord,currentYCoord).alpha;
				
				if((currentXCoord+1)<tgtW){
					xCoord.add(currentXCoord+1);
					yCoord.add(currentYCoord);
				}
				if((currentYCoord+1)<tgtH){
					xCoord.add(currentXCoord);
					yCoord.add(currentYCoord+1);
				}
				...
				if((done%frameFreq)==0) anim.addFrame(img);
			}
		}
	}
	return anim;
