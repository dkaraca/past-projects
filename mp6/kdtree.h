...
class KDTree
{
	...
	void quickselect(int left, int right, int k, int curDim);
	
	int partition(int left, int right, int pivotInd, int curDim);

	void KDTree_helper(int left, int right, int curDim);
	
	Point<Dim> findNearestNeighborHelper(const Point<Dim> & query, int left, int right, int curDim, const Point<Dim> &curBest) const;
