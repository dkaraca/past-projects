...
void HuffmanTree::buildTree( const vector<Frequency> & frequencies ) {
	...
	for(int i = 0; i<size; i++){
		TreeNode* single_Queue = new TreeNode(frequencies.at(i));
		singleQueue.push(single_Queue);
	}
	while(singleQueue.size() + mergeQueue.size() > 1){
		...
		TreeNode* tmp = new TreeNode((smallest->freq.getFrequency()) + (smallest_2->freq.getFrequency()));
		tmp->left = smallest;
		tmp->right = smallest_2;
		mergeQueue.push(tmp);
	}
...
void HuffmanTree::decode( stringstream & ss, BinaryFileReader & bfile ) {
	TreeNode * curr = root;
	while( bfile.hasBits() ) {
		if(bfile.getNextBit() == 0) curr = curr->left;
		 else curr = curr->right;
		if((curr->left == NULL) && (curr->right == NULL)) {ss<< curr->freq.getCharacter(); curr = root;}
		/**
		 * @todo Your code here!
		 *
